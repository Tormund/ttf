...
{.emit: """
#if __cplusplus
#define CONST_CASTT(T, x) const_cast<T>(x)
#else
#define CONST_CASTT(T, x) (T)(x)
#endif
#define CASTFONTINFO(x) CONST_CASTT(stbtt_fontinfo*, x)
#define CASTBUFFERINFO(x) CONST_CASTT(NU8*, x)
""".}
...
proc stbtt_InitFont*(info: ptr stbtt_fontinfo, data: ptr font_type, fontstart: cint): cint {.exportc.}
...
# proc stbtt_GetGlyphBitmapBox*(info: ptr stbtt_fontinfo, glyph: cint, scale_x, scale_y: cfloat, ix0, iy0, ix1, iy1: var cint) {.importc.}
...
proc stbtt_GetGlyphShape*(info: ptr stbtt_fontinfo, glyph_index: cint): seq[stbtt_vertex]

proc stbtt_GetCodepointShape*(info: ptr stbtt_fontinfo, unicode_codepoint: cint): seq[stbtt_vertex] =
...
proc stbtt_GetGlyfOffset(info: ptr stbtt_fontinfo, glyph_index: cint): cint {.exportc.} =
...
proc stbtt_GetGlyphBox*(info: ptr stbtt_fontinfo, glyph_index: cint, x0, y0, x1, y1: ptr cint): bool =
...
proc stbtt_IsGlyphEmpty*(info: ptr stbtt_fontinfo, glyph_index: cint): bool =
...
proc stbtt_GetCodepointBox*(info: ptr stbtt_fontinfo, codepoint: cint, x0, y0, x1, y1: ptr cint): bool =
...
proc stbtt_GetGlyphHMetrics*(info: ptr stbtt_fontinfo, glyph_index: cint, advanceWidth, leftSideBearing: ptr cint) {.exportc.} =
...
proc stbtt_GetGlyphKernAdvance*(info: ptr stbtt_fontinfo, glyph1, glyph2: cint): cint =
...
proc stbtt_GetCodepointHMetrics*(info: ptr stbtt_fontinfo, codepoint: cint, advanceWidth, leftSideBearing: ptr cint) =
    # leftSideBearing is the offset from the current horizontal position to the left edge of the character
    # advanceWidth is the offset from the current horizontal position to the next horizontal position
    # these are expressed in unscaled coordinates
    stbtt_GetGlyphHMetrics(info, stbtt_FindGlyphIndex(info,codepoint), advanceWidth, leftSideBearing)

proc stbtt_GetFontVMetrics*(info: ptr stbtt_fontinfo, ascent, descent, lineGap: var cint) =
...
proc stbtt_GetFontBoundingBox*(info: ptr stbtt_fontinfo, x0, y0, x1, y1: var int) =
...
proc stbtt_ScaleForPixelHeight*(info: ptr stbtt_fontinfo, height: cfloat): cfloat {.exportc.} =
...
proc stbtt_ScaleForMappingEmToPixels*(info: ptr stbtt_fontinfo, pixels: cfloat): cfloat {.exportc.} =
...
proc stbtt_GetBakedQuad*(bakedChar: ptr stbtt_bakedchar, pw, ph: int,  # same data as above
...
proc stbtt_InitFont*(info: ptr stbtt_fontinfo, data: ptr font_type, fontstart: cint): cint =
...
#
proc stbtt_GetGlyphBitmapBoxSubpixel(info: ptr stbtt_fontinfo, glyph: cint, scale_x, scale_y, shift_x, shift_y: cfloat, ix0, iy0, ix1, iy1: ptr cint) =
...
proc stbtt_GetGlyphBitmapBox*(info: ptr stbtt_fontinfo, glyph: cint, scale_x, scale_y: cfloat, ix0, iy0, ix1, iy1: ptr cint) {.exportc.} =
   stbtt_GetGlyphBitmapBoxSubpixel(info, glyph, scale_x, scale_y, 0.0, 0.0, ix0, iy0, ix1, iy1)

proc stbtt_GetCodepointBitmapBoxSubpixel(info: ptr stbtt_fontinfo, codepoint: cint, scale_x, scale_y, shift_x, shift_y: cfloat, ix0, iy0, ix1, iy1: ptr cint) {.exportc.} =
...
proc stbtt_GetCodepointBitmapBox*(info: ptr stbtt_fontinfo, codepoint: cint, scale_x, scale_y: cfloat, ix0, iy0, ix1, iy1: ptr cint) =
...
proc stbtt_rasterize_sorted_edges(result: ptr stbtt_bitmap, e: ptr stbtt_edge, n, vsubsample, off_x, off_y: cint, userdata: pointer) =
  {.emit: """
...
proc stbtt_rasterize(result: ptr stbtt_bitmap, ptsArr: openarray[stbtt_point], wcountArr: openarray[cint], scale_x, scale_y, shift_x, shift_y: cfloat, off_x, off_y, invert: cint, userdata: pointer) {.exportc.} =
...
proc stbtt_Rasterize(
       result: ptr stbtt_bitmap,           # 1-channel bitmap to draw into
...
proc stbtt_GetGlyphBitmapSubpixel(info: ptr stbtt_fontinfo, scale_x, scale_y, shift_x, shift_y: cfloat, glyph: cint, width, height, xoff, yoff: ptr cint): ptr uint8 {.exportc.} =
...
proc stbtt_GetGlyphBitmap*(info: ptr stbtt_fontinfo, scale_x, scale_y: cfloat, glyph: cint, width, height, xoff, yoff: ptr cint): ptr uint8 =
   # {.emit: """
   #    return `stbtt_GetGlyphBitmapSubpixel`(CASTBUFFERINFO(`info`), `scale_x`, `scale_y`, 0.0, 0.0, `glyph`, `width`, `height`, `xoff`, `yoff`)
   #  """.}
   return stbtt_GetGlyphBitmapSubpixel(info, scale_x, scale_y, 0.0, 0.0, glyph, width, height, xoff, yoff)

proc stbtt_MakeGlyphBitmapSubpixel(info: ptr stbtt_fontinfo, output: ptr uint8, out_w, out_h, out_stride: cint, scale_x, scale_y, shift_x, shift_y: cfloat, glyph: cint) {.exportc.} =
   var ix0, iy0: cint
   var vertices = stbtt_GetGlyphShape(info, glyph)
   var gbm : stbtt_bitmap
...
proc stbtt_MakeGlyphBitmap*(info: ptr stbtt_fontinfo, output: ptr byte, out_w, out_h, out_stride: cint, scale_x, scale_y: cfloat, glyph: cint) {.exportc.} =
...
proc stbrp_pack_rects(con: ptr stbrp_context, rects: var openarray[stbrp_rect]) {.exportc.} =
...
int main(int argc, char **argv)
{
   ...
   stbtt_InitFont &font, CASTBUFFERINFO(ttf_buffer), stbtt_GetFontOffsetForIndex(CASTBUFFERINFO(ttf_buffer),0));
...
int main(int arg, char **argv)
{
   ...
   stbtt_InitFont(&font, CASTBUFFERINFO(buffer), 0);
...
{
   return stbtt_GetGlyphBitmapSubpixel(CASTFONTINFO(info), scale_x, scale_y,shift_x,shift_y, stbtt_FindGlyphIndex(CASTFONTINFO(info),codepoint), width,height,xoff,yoff);
}
...
{
   stbtt_MakeGlyphBitmapSubpixel(CASTFONTINFO(info), output, out_w, out_h, out_stride, scale_x, scale_y, shift_x, shift_y, stbtt_FindGlyphIndex(CASTFONTINFO(info),codepoint));
}
                                ...
                                stbtt_bakedchar *chardata)
{
   ...
   int x,y,bottom_y, i;
   stbtt_fontinfo f;
   if (!stbtt_InitFont(&f, CASTBUFFERINFO(data), offset))
      return -1;
   ...
   for (i=0; i < num_chars; ++i) {
      ...
      int g = stbtt_FindGlyphIndex(CASTFONTINFO(&f), first_char + i);
...
int stbtt_PackFontRanges(stbtt_pack_context *spc, unsigned char *fontdata, int font_index, stbtt_pack_range *ranges, int num_ranges)
{
   ...
   stbtt_InitFont(&info, CASTBUFFERINFO(fontdata), stbtt_GetFontOffsetForIndex(CASTBUFFERINFO(fontdata),font_index));
   k=0;
   for (i=0; i < num_ranges; ++i) {
   ...
   for (i=0; i < num_ranges; ++i) {
      ...
      for (j=0; j < ranges[i].num_chars_in_range; ++j) {
         ...
         if (r->was_packed) {
            ...
            int advance, lsb, x0,y0,x1,y1;
            int glyph = stbtt_FindGlyphIndex(CASTFONTINFO(&info), ranges[i].first_unicode_char_in_range + j);
...
int stbtt_FindMatchingFont(const unsigned char *font_collection, const char *name_utf8, stbtt_int32 flags)
{
   stbtt_int32 i;
   for (i=0;;++i) {
      stbtt_int32 off = stbtt_GetFontOffsetForIndex(CASTBUFFERINFO(font_collection), i);
      if (off < 0) return off;
